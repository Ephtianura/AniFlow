...
**AniFlow** – вебплатформа для перегляду аніме українською мовою. З автоматичною синхронізацією бази контенту через партнерів, SSR-рендерингом для SEO, багаторівневим кешуванням, соціальними функціями, кастомною адмінкою з метриками та хмарною інфраструктурою.
...
<summary>Навіщо AniFlow?</summary>

Це платформа, яка з'явилася з любові до аніме та прагнення зробити щось дійсно якісне для української спільноти, включаючи розповсюдження якісного україномовного контенту. Проєкт починався в першу чергу для себе, для власних потреб і щоб навчитися створювати складні рішення та готові до життя системи з нуля, з прагненням прибрати весь комерційний хаос та недоліки на подібних сайтах, взяти все найкраще й зробити це ще кращим. Тому кожній, на перший погляд, непомітній дрібниці приділено особливу увагу та полірування.

Сподіваюся, я зможу цим репозиторієм показати, що навіть один розробник може створити охайну, зручну та автономну платформу для людей.
...
![](https://img.shields.io/badge/Cloudflare-F38020?style=flat-square&logo=cloudflare&logoColor=white) <!-- Cloudflare -->
...
<summary><h2 id="architecture">📖 Архітектура</h2></summary>

---

<img alt="aniFlow-architecture-diagram" src=".github/assets/readme/aniFlow-architecture-diagram.webp" />
<p align="center"><small><i>Архітектурна діаграма проєкту.</i></small></p>

Бекенд-частина платформи спроектована як модульний моноліт.

В основі лежать принципи **Clean Architecture** із розділенням системи на незалежні шари:
...
Вся інфраструктура проєкту разом із фронтендом контейнеризована через **Docker**. Фінальне розгортання реалізовано на виділеному сервері, де як єдина точка входу та зворотний проксі-сервер виступає **Nginx**, який забезпечує маршрутизацію трафіку та кешування.
...
Початково фронтенд створювався як класичний SPA, проте через мерехтіння сторінок та низьку ефективність для пошукових ботів, архітектуру було повністю переосмислено. Тепер усі ключові сторінки працюють виключно через SSR (Server-Side Rendering) для якісної SEO-оптимізації та плавності. Завдяки Next.js усе побудовано на базі App Router та Island Architecture, а отримання даних є асинхронним там, де це доречно. Також згенеровано sitemap для кращої індексації. Перехід на SSR знизив середню швидкість завантаження сторінок на **700 мс**.
...
Оскільки мені потрібна була максимальна кастомізація інтерфейсу - ідеальним вибором став TailwindCSS. Динаміку та плавність елементів забезпечує бібліотека Motion. Сортування скриншотів реалізовано за допомогою dnd kit, а на бекенді цей функціонал підтримується алгоритмом Fractional Indexing. Для кастомізації специфічних елементів (чекбокси, випадаючі списки тощо..) я також не скупився на перевірені рішення на кшталт MUI.

### 🌐 Деплой

##### 🔁 Проксі

Як легкий та зручний зворотний проксі використано Nginx Proxy Manager, де також налаштовано випуск SSL-сертифікатів. Поруч із цим я експериментував і з чистим Nginx для глибшого розуміння процесів.

Перед сервером також використовується Cloudflare, який забезпечує захист від DDoS-атак, приховує реальну IP-адресу сервера, кешує статичний контент та виконує роль додаткового рівня безпеки й оптимізації мережевого трафіку.
...
Як доменного реєстратора обрано Porkbun, де я самостійно конфігурував DNS-записи та поштові сервіси, пізніше перейшовши на Cloudflare.
...
### ☁️ Впровадження CI/CD, перехід на AWS S3 та Cloudflare
...
Платформу успішно розгорнуто на серверах Hetzner. Проведено комплексне тестування за участю **13 реальних користувачів**. На основі їхнього фідбеку система пройшла етап стабілізації, було усунуто критичні баги та оптимізовано загальну роботу додатка.
...
- ✨ **Split-mode редагування** – режим подвійного екрана, у якому поруч із формою редагування відображатиметься інформація про тайтл із MyAnimeList. Це дозволить адміністраторам швидко переносити опис, характеристики та інші дані без постійного перемикання між вкладками та копіювання, значно пришвидшуючи процес наповнення каталогу.
...
- ♻️ **Soft Delete** – м'яке видалення з приховуванням без повного знищення в базі даних.
