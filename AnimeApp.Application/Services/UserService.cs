...
{
    public class UserService(IUserRepository users, IPasswordHasher passwordHasher) : IUserService

    {
        ...
        public async Task AddOrUpdateAnimeAsync(int userId, Anime anime, MyListEnum? list = null, int? rating = null)
        {
            var user = await GetUserWithAnimeByIdAsync(userId);
            ...
            if (userAnime == null)
            {
                ...
                if (list.HasValue)
                    userAnime.ChangeMyList(list.Value);

                if (rating.HasValue)
                    userAnime.ChangeRating(rating);
        ...
        //        user.UpdateAvatarFileName(posterFileName);
        //    }
        ...
        //                user.UpdateAvatarFileName(saved);
        //            }
        //        }
        //        catch
        //        {
